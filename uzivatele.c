...
void gradeStudents(const char* teacherUsername) {
    ...
    int weight;
    do {
        printf("Zadejte vahu (1-3): ");
        if (scanf("%d", &weight) == 1 && weight >= 1 && weight <= 3) {
            break;
        }
    ...
    for (int i = 0; i < selectedCount; i++) {
        ...
        snprintf(gradeEntry, sizeof(gradeEntry), "%sz%dv%d%s", prefix, grade, weight, selectedGroup);
    ...
    printf("Znamka %s%d s vahou %d byla prirazena studentum ve skupine %s.\n", prefix, grade, weight, selectedGroup);
    saveUsers();
}
        ...
        if (users[i].role == 'T' && strstr(users[i].subjects, selectedGroup)) { // Pouze studenti v dané skupinì
            ...
            while (token != NULL) {
                ...
                if (tokenLen > groupLen && strcmp(&token[tokenLen - groupLen], selectedGroup) == 0) {
                    ...
                    int grade, weight;
                    if (sscanf(token, "%*[^z]z%dv%d", &grade, &weight) == 2) { // Ignoruje prefix (cv, t, zk)
            ...
            if (first) { // Pokud student nemá žádné známky pro skupinu
                ...
                if (totalWeight > 0) {
                    ...
                    // Urèení výsledné známky
                    char finalGrade = 'F';
                    if (average >= 91) finalGrade = 'A';
                    else if (average >= 81) finalGrade = 'B';
                    else if (average >= 71) finalGrade = 'C';
                    else if (average >= 61) finalGrade = 'D';
                    else if (average >= 51) finalGrade = 'E';

                    printf(" | Vysledna znamka: %c", finalGrade);
                }
            }
            printf("\n");
            studentCount++;
...
//!!!! upravit zobrazování známek u uèitele



//verze zobrazení známek u studentu s vahou a prumerem
/*
void viewStudentGrades(const char* studentUsername) {
    // Krok 1: Výpis skupin studenta
    printf("Vase studijni skupiny:\n");
